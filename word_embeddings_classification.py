...
def extract_features(embedding_size,utterances,embd_df):
	X=np.empty(shape=[0, 2*embedding_size])
	Y=[]

	#get number of lines
	num_lines = sum(1 for line in open(utterances))
	...
	while line:
		...
		utternace=utterance.strip();

		words=utterance.split();	

		arr = np.empty(shape=[0, embedding_size])
	
		#put all embeddings for this sentence in an array
		for word in words:
			arr=np.append(arr,get_embedding(word,embd_df),axis=0)
		#get the min & max, and concatenate them.
		
		if arr.size==0:
			arr=np.zeros((2,embedding_size));
	
		#keep min and amx embedding as representative of sentence
		...
		#append to X,Y for training...
		X=np.append(X,np.reshape(feature,(1,2*embedding_size)),axis=0);
		Y.append(label);
		...
		# nts_int=int(nts);
		# sys.stdout.write('\r')
		# sys.stdout.write("[%-100s] %d%%" % ('='*nts_int, nts))
		# sys.stdout.flush()

		sys.stdout.write('\r')
		#sys.stdout.write("	Progress: [%d%%]" % (nts*100));
		sys.stdout.write("[%-100s] %d%%" % ('='*int(nts*100), nts*100))
		sys.stdout.flush()
		#print i," ",nts*100,"%";
...
def get_embedding(word,df):
	return df.loc[df['word'] == word, df.dtypes == float].as_matrix();
